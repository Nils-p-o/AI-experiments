...
from transformer_arch.money.money_enc_dec_MTP_aux_trade_loss_muP import Money_former_MLA_DINT_cog_attn_MTP
from training.data_loaders.test_feats_stocks_time_series_enc_dec_aux_trade_loss import (
    FinancialNumericalDataModule,
    download_numerical_financial_data,
)
from training.money_experiment_2_MTP_enc_dec_aux_trade_loss import MoneyExperiment
...
def proceed(args: argparse.Namespace):
    ...
    print(
        f"money_former_MLA_DINT_cog_attn enc dec seq_len:{seq_len} d_model:{d_model} d_ff:{d_ff} enc_layers:{num_enc_layers} dec_layers:{num_dec_layers} nhead:{nhead} dropout:{dropout} lr:{lr} t_total:{t_total} warmup_steps:{warmup_steps} t_0:{t_0} t_mult:{t_mult} lr_mult:{lr_mult} batch_size:{batch_size}"
    )

    name = f"{args.dataset}/{args.folder_name}/{architecture}_{seq_len}_{d_model}_{d_ff}_{num_enc_layers}_{num_dec_layers}_{nhead}_{batch_size}"
    ...
    if args.dataset == "Money":  # yahoo finance stock data
        ...
        data_module = FinancialNumericalDataModule(
            ...
            aux_train_file="time_series_data/train_aux.pt",
            aux_train_targets_file="time_series_data/train_aux_MTP_targets.pt",
            aux_val_file="time_series_data/val_aux.pt",
            aux_val_targets_file="time_series_data/val_aux_MTP_targets.pt",
            aux_test_file="time_series_data/test_aux.pt",
            aux_test_targets_file="time_series_data/test_aux_MTP_targets.pt",
            seq_len=seq_len,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./experiment_configs/MTP_enc_dec_aux_trade_loss_exp_base.json",
