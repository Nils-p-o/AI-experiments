# for model that extracts its own engineered features (i still have to supply some that are difficult to extract)
...
def download_numerical_financial_data(
    ...
    # global_data, local_data, time_data = torch.split(full_data, [len(columns), len(local_columns), len(time_columns)], dim=0)
    ...
    #     full_data = torch.cat((global_data, local_data, time_data), dim=0)
    ...
    data, fitted_processors = auto_correct_feature_skew_pre_znorm(data, column_to_id, train_data_length+stem_seq_len-1, num_of_non_global_norm_feats, len(time_columns), stem_seq_len)
    ...
    if config_args.n_noisy_copies > 0:
        ...
        for copy in noisy_copies:
            ...
            copy = copy.unfold(2, stem_seq_len, 1).contiguous()
