...
def download_numerical_financial_data(
    ...
    data = torch.cat(data, dim=-1)  # (features, time series, tickers)
    ...
    data[len(target_dates)+4, 5909] = data[
        len(target_dates) + 4, 5908
    ...
    # global z-normalization (diff)
    # means_close_ret = train_data[len(target_dates):len(target_dates)+1].mean(dim=1, keepdim=True)
    # stds_close_ret = train_data[len(target_dates):len(target_dates)+1].std(dim=1, keepdim=True)
    # mean_vol_ret = train_data[len(target_dates)+4:len(target_dates)+5].mean(dim=1, keepdim=True)
    # std_vol_ret = train_data[len(target_dates)+4:len(target_dates)+5].std(dim=1, keepdim=True)
    # means_close_price = train_data[len(target_dates)+5:len(target_dates)+6].mean(dim=1, keepdim=True)
    # stds_close_price = train_data[len(target_dates)+5:len(target_dates)+6].std(dim=1, keepdim=True)
    # mean_vol = train_data[len(target_dates)+9:len(target_dates)+10].mean(dim=1, keepdim=True)
    # std_vol = train_data[len(target_dates)+9:len(target_dates)+10].std(dim=1, keepdim=True)
    # means = torch.cat([means_close_ret.repeat(4,1,1),mean_vol_ret.repeat(1,1,1), means_close_price.repeat(4,1,1), mean_vol.repeat(1,1,1)], dim=0)
    # stds = torch.cat([stds_close_ret.repeat(4,1,1),std_vol_ret.repeat(1,1,1), stds_close_price.repeat(4,1,1), std_vol.repeat(1,1,1)], dim=0)
    # means = torch.cat([means_close_ret.repeat(4, 1, 1), mean_vol_ret], dim=0)
    # stds = torch.cat([stds_close_ret.repeat(4, 1, 1), std_vol_ret], dim=0)
    # train_data[len(target_dates):-15] = (train_data[len(target_dates):-15] - means) / stds
    # val_data[len(target_dates):-15] = (val_data[len(target_dates):-15] - means) / stds
    # test_data[len(target_dates):-15] = (test_data[len(target_dates):-15] - means) / stds

    # means = train_data[len(target_dates) : len(target_dates) + 4].mean(dim=(0,1), keepdim=True)
    # stds = train_data[len(target_dates) : len(target_dates) + 4].std(dim=(0,1), keepdim=True)
    # vol_mean = train_data[len(target_dates) + 4 : len(target_dates) + 5].mean(dim=(1), keepdim=True)
    # vol_std = train_data[len(target_dates) + 4 : len(target_dates) + 5].std(dim=(1), keepdim=True)
    # means = torch.cat([means.repeat(4, 1, 1), vol_mean.repeat(1, 1, 1)], dim=0)
    # stds = torch.cat([stds.repeat(4, 1, 1), vol_std.repeat(1, 1, 1)], dim=0)
    means = train_data[len(target_dates) : -15].mean(dim=1, keepdim=True)
    stds = train_data[len(target_dates) : -15].std(dim=1, keepdim=True)
    train_data[len(target_dates) : -15] = (train_data[len(target_dates) : -15] - means) / stds
    val_data[len(target_dates) : -15] = (val_data[len(target_dates) : -15] - means) / stds
    test_data[len(target_dates) : -15] = (test_data[len(target_dates) : -15] - means) / stds
