    ...
    def _shared_step(self, batch, batch_idx, stage="train"):
        ...
        match self.args.prediction_type:
            ...
            case "classification":
                ...
                match self.args.cost_type:
                    case "exp_mult":
                        batch_cost_weights = torch.exp(batch_cost_weights)
                    ...
                    case "exp_add":
                        ...
                        loss = (loss_tensor * self.loss_weights * class_weights + batch_cost_weights).mean()
                    case "lin_mult":
                        batch_cost_weights = batch_cost_weights + 1
                        loss = (loss_tensor * self.loss_weights * class_weights * batch_cost_weights).mean()
                    case "lin_add":
                        loss = (loss_tensor * self.loss_weights * class_weights + batch_cost_weights).mean()
                # loss_tensor = loss_tensor + batch_cost_weights

                # loss = (loss_tensor * self.loss_weights * class_weights).mean()
