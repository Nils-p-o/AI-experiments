...
def dummy_dataloader(batch_size=32, seq_length=128, num_samples=1000, model_type="TransformerEncoder", config=None):
    # Adjust dummy data creation based on model type
    if model_type == "EncoderDecoder" or (model_type == "MultiTransformer" and "decoder" in config):
        ...
        labels = torch.randint(0, 10, (num_samples, seq_length)) # Assuming sequence classification
        ...
        if "decoder" in config:
            decoder_input_ids = torch.randint(0, 30000, (num_samples, seq_length))
            decoder_attention_mask = torch.ones(num_samples, seq_length)
            labels = torch.randint(0, 10, (num_samples, seq_length)) # Example for sequence generation
            dataset = TensorDataset(encoders_data, (decoder_input_ids, decoder_attention_mask), labels)
        else:
...
if __name__ == '__main__':
    ...
    data_module.setup() #need to setup!

    # Example of getting a batch
    train_dataloader = data_module.train_dataloader()
    for batch in train_dataloader:  # Iterate to get the first batch
        inputs, targets = batch
        print("Inputs shape:", inputs.shape)  # Should be (batch_size, seq_len)
        print("Targets shape:", targets.shape) # Should be (batch_size, seq_len)

        # Decode a sample sequence to check the shifting
        print("Sample Input:", data_module.train_dataset.decode(inputs[0].tolist()))
        print("Sample Target:", data_module.train_dataset.decode(targets[0].tolist()))
        break  # Only get one batch for demonstration
