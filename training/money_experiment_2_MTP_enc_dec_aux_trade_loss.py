...
def calculate_expected_accuracy_finer(targets_classes, num_classes):
    accuracy = 0.0
    occurances = torch.empty((targets_classes.shape[-1], num_classes), device=targets_classes.device)
    for i in range(num_classes):
        for j in range(targets_classes.shape[-1]):
            occurances[j,i] = (targets_classes[:,:,:,j] == i).sum()
    max_occurances = occurances.max(dim=-1).values
    accuracy = max_occurances.sum() / targets_classes.numel()
    return accuracy
...
def calculate_trading_metrics(returns):
    ...
    cummax = (1 + returns).cumprod(dim=0).cummax(dim=0).values
    drawdown = ((1 + returns).cumprod(dim=0) / cummax - 1).abs()
    max_drawdown = drawdown.max(dim=0).values

    annualized_volatility = returns.std() * (252 ** 0.5)
    sharpe_ratio = annualized_return / annualized_volatility if annualized_volatility != 0 else 0.0
    sortino_ratio = (
        annualized_return / (returns[returns < 0].std() * (252 ** 0.5))
    ...
    win_loss_ratio = (returns > 0).sum() / (returns < 0).sum() if (returns < 0).sum() != 0 else 1.0
...
class MoneyExperiment(pl.LightningModule):
    ...
    def _shared_step(self, batch, batch_idx, stage="train"):
        ...
        full_inputs, full_labels = batch
        inputs, aux_inputs = full_inputs
        labels, _ = full_labels
        ...
        aux_inputs = aux_inputs.permute(0, 2, 3, 4, 1).contiguous()
        ...
        aux_tickers = torch.arange(self.num_aux_sequences, device=inputs.device)
        aux_tickers = aux_tickers.unsqueeze(0).unsqueeze(0).repeat(
            batch_size, len(self.pred_indices), 1
        )
        if stage == "train":
        ...
        trades, cash = self(inputs, aux_inputs, tickers, aux_tickers)
        ...
        if torch.isnan(cash).any():
            print("NAN IN CONFIRMATIONS")
        ...
        match self.args.prediction_type:
            ...
            case "portfolio":
                ...
                portfolio_weights = torch.softmax(raw_weights, dim=-1)
                stock_directions = torch.tanh(raw_directions)
                stock_directions = torch.cat([stock_directions, torch.ones_like(stock_directions[:,:,:,:1])], dim=-1) # for cash

                mock_weights = portfolio_weights * stock_directions

                target_gross_exposure = 1.0
                current_gross_exposure = torch.sum(torch.abs(mock_weights), dim=-1, keepdim=True) # (batch_size, seq_len, targets, 1)
                final_weights = target_gross_exposure * (mock_weights / (current_gross_exposure + 1e-8)) # (batch_size, seq_len, targets, num_sequences+1)
                # final_weights = mock_weights

                prev_weights = torch.roll(final_weights, shifts=1, dims=1) # (batch_size, seq_len, targets, num_sequences+1)
                prev_weights[:,0,:,:] = 0.0
                prev_weights[:,0,:,-1:] = 1.0 # set beginning as all cash

                transaction_cost_rate = 0.0005
                turnover = torch.sum(torch.abs(final_weights - prev_weights), dim=-1) # (batch_size, seq_len, targets)
                transaction_costs = transaction_cost_rate * turnover # (batch_size, seq_len, targets)
        ...
        if calculate_detailed_metrics:
            ...
            with torch.no_grad():
                ...
                if self.args.prediction_type == "portfolio":
                    ...
                    preds_classes = torch.full_like(stock_directions[:,:,:,:-1], 1)
                    ...
                    if stage == "val":
                        ...
                        for i in range(len(self.tickers)):
                            self.log( # final weights
                                f"Portfolio_weights/{stage}_seen_weight_{self.tickers[i]}",
                                final_weights[:, :-1, :, i].abs().mean(),
                                **current_log_opts,
                            )
                            self.log(
                                f"Portfolio_weights/{stage}_unseen_weight_{self.tickers[i]}",
                                final_weights[:, -1:, :, i].abs().mean(),
                                **current_log_opts,
                            )
                    ...
                    for i in range(len(self.tickers)):
                        ...
                        self.log(
                            f"Sequence/{stage}_seen_accuracy_bin_{self.tickers[i]}",
                        ...
                        self.log(
                            f"Sequence/{stage}_unseen_accuracy_bin_{self.tickers[i]}",
                            unseen_target_accuracy,
                            **current_log_opts,
                        )
                    
                    unseen_targets_class = targets_classes[:, -1:, :, :]
                    unseen_preds_class = preds_classes[:, -1:, :, :]
                    confusion_matrix = torch.zeros(2, 2, device=targets.device)
                    for i in range(2):
                        for j in range(2):
                            confusion_matrix[i, j] = ((unseen_targets_class == i) & (unseen_preds_class == j)).sum()
                    
                    unseen_accuracy = (
                        confusion_matrix.diag().sum() / confusion_matrix.sum()
                    )
                    self.log(
                        f"Confusion/{stage}_unseen_accuracy",
                        unseen_accuracy,
                        **current_log_opts,
                    )

                    unseen_precisions = torch.zeros(2, device=targets.device)
                    unseen_recalls = torch.zeros(2, device=targets.device)
                    unseen_f1s = torch.zeros(2, device=targets.device)
                    ...
                    for i in range(2):
                        ...
                        unseen_precisions[i] = true_positives / (true_positives + false_positives + 1e-8)
                        unseen_recalls[i] = true_positives / (true_positives + false_negatives + 1e-8)
                        unseen_f1s[i] = 2 * (unseen_precisions[i] * unseen_recalls[i]) / (unseen_precisions[i] + unseen_recalls[i] + 1e-8)

                        self.log(
                            f"Confusion/{stage}_unseen_precision_class_{i}",
                            unseen_precisions[i],
                            **current_log_opts,
                        )
                        self.log(
                            f"Confusion/{stage}_unseen_recall_class_{i}",
                            unseen_recalls[i],
                            **current_log_opts,
                        )
                        self.log(
                            f"Confusion/{stage}_unseen_f1_class_{i}",
                            unseen_f1s[i],
                            **current_log_opts,
                        )
                    
                    self.log(
                        f"Confusion/{stage}_unseen_precision",
                        unseen_precisions.mean(),
                        **current_log_opts,
                    )

                    self.log(
                        f"Confusion/{stage}_unseen_recall",
                        unseen_recalls.mean(),
                        **current_log_opts,
                    )

                    self.log(
                        f"Confusion/{stage}_unseen_f1",
                        unseen_f1s.mean(),
                        **current_log_opts,
                    )
                    ...
                    self.log(
                        f"Portfolio/{stage}_cash",
                        portfolio_weights[:, :, :, -1].mean(),
                        **current_log_opts,
                    )

                    self.log(
                        f"Portfolio/{stage}_unseen_cash",
                        portfolio_weights[:, -1, :, -1].mean(),
                        **current_log_opts,
                    )

                    self.log(
                        f"Portfolio/{stage}_seen_direction_conf",
                        stock_directions[:,:-1,:,:-1].abs().mean(),
                        **current_log_opts,
                    )
                    self.log(
                        f"Portfolio/{stage}_unseen_direction_conf",
                        stock_directions[:,-1:,:,:-1].abs().mean(),
                        **current_log_opts,
                    )

                    self.log(
                        f"Portfolio/{stage}_seen_period_returns",
                        period_returns[:,:-1,:].mean(),
                        **current_log_opts,
                    )

                    self.log(
                        f"Portfolio/{stage}_unseen_period_returns",
                        period_returns[:,-1:,:].mean(),
                        **current_log_opts,
                    )

                    self.log(
                        f"Portfolio/{stage}_unseen_mock_sharpe",
    ...
    def configure_optimizers(self):
        ...
        if self.args.optimizer == 'muon':
            ...
            non_muon_linear_weights = {
                'model.main_input.weight',
                'model.out.weight',
                'model.aux_input.weight',
    ...
    def on_validation_epoch_end(self):
        ...
        if self.trainer.is_global_zero:
            ...
            for indice in self.pred_indices:
                backtest_metrics = calculate_trading_metrics(all_returns[:,indice-1])
                for metric_name, metric_value in backtest_metrics.items():
                    self.log(
                        f"Strategy_metrics_{indice}/val_{metric_name}",
                        metric_value,
                        **log_opts_epoch,
                    )
                self.log(
