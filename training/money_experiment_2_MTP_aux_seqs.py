...
class MoneyExperiment(pl.LightningModule):
    def __init__(
        ...
        self.full_tickers = args.tickers + args.auxilary_tickers
    ...
    def _shared_step(self, batch, batch_idx, stage="train"):
        ...
        if calculate_detailed_metrics:
            ...
            for i in range(self.num_sequences):
                ...
                self.log(
                    f"Split_Error/{stage}_seen_{self.full_tickers[i]}_MAE",
                    seen_MAE,
                    **current_log_opts,
                )
                self.log(
                    f"Split_Error/{stage}_unseen_{self.full_tickers[i]}_MAE",
                    unseen_MAE,
                    **current_log_opts,
                )
