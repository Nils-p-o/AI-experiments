...
class LLaMa_MLA(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                LLaMa_MLA_block(args)
                for _ in range(self.num_layers)
            ]
        )
        self.input_embedding = input_embedding(self.d_model, vocab_size)
...
class MLA_simple(nn.Module): # TODO make optimised version with "absorbed" weights + qkv norm
    def __init__(self, args):
        super().__init__()
        self.d_model = args.d_model
        self.nhead = args.nhead
        ...
        self.kv_compression_dim = self.d_model // 4 #TODO check how much this matters, should be a hyperparameter (roughly what R1 has)
        # probably stops mattering after some point
        self.q_compression_dim = self.kv_compression_dim * 2 #TODO check how much this matters, should be a hyperparameter (roughly what R1 has)
    ...
    def forward(self, x, mask=None):
        ...
        # )
        a = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim + self.rope_dim)
