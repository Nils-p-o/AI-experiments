...
class Money_former_MLA_DINT_cog_attn_MTP(nn.Module):
    ...
    def forward(
        ...
        if self.unique_outputs:
            ...
            confirmation_input = x_end[:, :, 1:, :].view(batch_size, targets, num_sequences, seq_len, -1)
            confirmation_input = confirmation_input.permute(0, 1, 3, 2, 4).contiguous()
            confirmation_input = confirmation_input.view(batch_size, targets, seq_len, -1)
...
class custom_MHA(nn.Module): # a mix of MLA and DINT
    ...
    def forward(self, x, mask=None, freqs_cis=None):
        ...
        G_vec = torch.mean(a[:, :, 0, :, :], dim=-2, keepdim=True)  # (batch_size, nhead, 1, seq_len)
