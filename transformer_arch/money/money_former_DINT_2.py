...
class Money_former_DINT(nn.Module):
    ...
    def forward(self, x, seperator=None, tickers=None): # seperator (batch_size, 1); tickers (batch_size, num_sequences)
...
class Money_former_block(nn.Module):
    ...
    def forward(self, x, freqs_cis):
        ...
        mask = mask.repeat(batch_size, 2*self.nhead, self.num_sequences, self.num_sequences) # repeat, or tile?
...
class DINT_MHA(nn.Module):
    ...
    def forward(self, x, mask=None, freqs_cis=None):
        ...
        # TODO reshaping apparently?
        # --- Q, K, V Projections ---
        # q = self.q_linear(x).view(
        #     batch_size, seq_per_stock, self.nhead * 2, self.head_dim // 2, self.num_sequences
        # )
        ...
        q = q_proj.view(batch_size, seq_per_stock, self.num_sequences, self.nhead * 2, self.head_dim // 2).permute(0, 1, 3, 4, 2)
        k = k_proj.view(batch_size, seq_per_stock, self.num_sequences, self.nhead * 2, self.head_dim // 2).permute(0, 1, 3, 4, 2)
        ...
        q = q_perm.reshape(batch_size, seq_len, self.nhead*2, self.head_dim//2) # q_final shape: (batch, nhead*2, L_flat, head_dim//2)
        k = k_perm.reshape(batch_size, seq_len, self.nhead*2, self.head_dim//2)

        # q = q.view(batch_size, -1, self.nhead * 2, self.head_dim // 2)
        # k = k.view(batch_size, -1, self.nhead * 2, self.head_dim // 2)
        ...
        # original code:
            ...
            a3 = G_vec
            # mask_seq_len = seq_len//self.num_sequences
            # a3_scaler = (mask_seq_len/(torch.arange(mask_seq_len)+1)).tile(batch_size,self.nhead,1).unsqueeze(-1)
            
            # a3 = torch.mean(a[:, :, 0, :mask_seq_len, :], dim=-1, keepdim=True) * a3_scaler
            # a3 = a3.repeat(1,1,self.num_sequences,mask_seq_len*self.num_sequences)
