...
class Money_former_MLA_DINT_cog_attn_MTP(nn.Module): # TODO MTP blocks
    def __init__(self, args):  # , dtype=torch.float32):
        ...
        self.ticker_embedding = nn.Embedding(len(args.tickers + args.auxilary_tickers), self.d_model)
        ...
        self.auxilary_input_dim = args.auxilary_input_features
        self.auxilary_input = nn.Linear(
            self.auxilary_input_dim, self.d_model, bias=args.bias
        )
        # TODO maybe embedding for whether stock or auxilary?
    ...
    def encode_inputs(self, x):
        ...
        standard_stock_x, auxilary_seq_x = torch.split(x, [len(self.args.tickers), len(self.args.auxilary_tickers)], dim=-2)
        auxilary_seq_x = torch.cat((auxilary_seq_x[:, :, :, :, :(self.auxilary_input_dim - 15)], auxilary_seq_x[:, :, :, :, -15:]), dim=-1)


        shared_temp_x = self.shared_value_input(standard_stock_x)
        aux_shared_temp_x = self.auxilary_input(auxilary_seq_x)
        x = torch.cat([shared_temp_x, aux_shared_temp_x], dim=-2)
...
class Money_former_block(nn.Module):
    def __init__(self, args, depth=0):
        ...
        self.num_sequences = len(args.tickers + args.auxilary_tickers)
