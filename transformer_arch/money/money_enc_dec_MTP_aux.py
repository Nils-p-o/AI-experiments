# encoder-decoder version of money_former_MLA_DINT_cog_attn_2_MTP_aux
...
class Money_former_MLA_DINT_cog_attn_MTP(nn.Module):
    def __init__(self, args):  # , dtype=torch.float32):
        ...
        self.encoder_stack = nn.ModuleList(
            [Encoder_layer(args, depth) for depth in range(self.enc_layers)]
        )

        self.decoder_stack = nn.ModuleList(
            [Decoder_layer(args, depth) for depth in range(self.dec_layers)]
        )
    ...
    def forward(
        ...
        if self.use_global_seperator:
            ...
            x_normal = (x_normal + main_ticker_embs) * self.scaling
            ...
            x_aux = (x_aux + aux_ticker_embs) * self.scaling
        else: # TODO adapt for enc dec
...
# TODO MTP encoder-decoder layer/block
# one encoder to freshen the info, one decoder to generate new info
...
class custom_MHA(nn.Module): # self_attn_module, that is a mix of MLA and DINT, cog attn
    def __init__(self, args, depth=0):
        super().__init__()
        self.d_model = args.d_model
        self.nhead = args.nhead
    ...
    def forward(self, x, mask=None, freqs_cis=None, cross_attn=False, enc_x=None, is_decoder=True):
        ...
        else: # TODO adapt for cross attn
        ...
        G_vec = torch.mean(a[:, :, 0, :, :], dim=-2, keepdim=True).repeat(1, 1, q_seq_len, 1)
