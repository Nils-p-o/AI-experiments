...
from transformer_arch.money.money_enc_dec_MTP_aux import Money_former_MLA_DINT_cog_attn_MTP
from training.data_loaders.test_feats_stocks_time_series_2_MTP_new_aux_rewrite import (
    FinancialNumericalDataModule,
    download_numerical_financial_data,
)
from training.money_experiment_2_MTP_enc_dec_aux import MoneyExperiment
...
def proceed(args: argparse.Namespace):
    ...
    print(
        f"money_former_MLA_DINT_cog_attn enc dec seq_len:{seq_len} d_model:{d_model} d_ff:{d_ff} num_layers:{num_layers} nhead:{nhead} dropout:{dropout} lr:{lr} t_total:{t_total} warmup_steps:{warmup_steps} t_0:{t_0} t_mult:{t_mult} lr_mult:{lr_mult} batch_size:{batch_size}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./experiment_configs/MTP_classification_exp_enc_dec_aux.json",
