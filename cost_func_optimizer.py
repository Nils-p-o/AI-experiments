...
from training.money_experiment_2_MTP_loss_opt import MoneyExperiment
...
def objective(trial: optuna.trial.Trial, base_config_path: str, data_module: FinancialNumericalDataModule):
    ...
    args.up_down_err_w = trial.suggest_float('up_down_err_w', 0.0, 5.0)
    args.flat_p_err_w = trial.suggest_float('flat_p_err_w', 0.0, 3.0)
    args.flat_t_err_w = trial.suggest_float('flat_t_err_w', 0.0, 3.0)

    args.cost_type = trial.suggest_categorical(
        'cost_type', ['exp_mult', 'exp_add', 'lin_mult', 'lin_add']
    )
    ...
    class PruningCallback(pl.Callback):
        def on_validation_end(self, trainer, pl_module):
            ...
            current_loss = trainer.callback_metrics.get("Backtest_metrics/val_Total_F1")
    ...
    logger = TensorBoardLogger("HPO_Logs", name=f"cost/trial_{trial.number}")
    early_stopping_callback = EarlyStopping(
        monitor="Backtest_metrics/val_Total_F1", patience=5, mode="min"
    )
    ...
    return trainer.callback_metrics.get("Backtest_metrics/val_Total_F1", float('inf')).item()
...
def run_hpo():
    ...
    study_name = "money-former-cost-v1"  # Give your study a version name
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="./experiment_configs/hpo_classification_base.json"
    )
