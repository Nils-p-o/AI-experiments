...
def download_numerical_financial_data(
    ...
    # TODO supposed to be only train data
    ...
    if train_data_length > 0:
        val_data_length = data_length - train_data_length - test_data_length
    else:
    ...
    data, fitted_processors = auto_correct_feature_skew_pre_znorm(data, train_data_length+seq_len-1, len(time_columns), seq_len)
    ...
    data = auto_correct_feature_kurtosis_post_znorm(data, train_data_length)
    ...
    return (train_indexes, val_indexes, test_indexes), (train_data, val_data, test_data), (train_MTP_targets, val_MTP_targets, test_MTP_targets), (means, stds), metadata
...
def download_and_save_numerical_financial_data(    
    tickers: List[str],
    seq_len: int = 64,
    target_dates: List[str] = [1],
    ...
    train_MTP_targets, val_MTP_targets, test_MTP_targets = MTP_targets
...
def auto_correct_feature_skew_pre_znorm(data: torch.Tensor, data_length: int, time_feats: int, seq_len: int, skew_threshold: float=1.0) -> torch.Tensor:
...
def auto_correct_feature_kurtosis_post_znorm(data: torch.Tensor, data_length: int) -> torch.Tensor:
